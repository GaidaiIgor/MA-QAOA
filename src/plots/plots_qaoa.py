...
def plot_methods_ar_9_nodes_general(methods: list[str], labels: list[str], arg_x_func: callable, **kwargs):
    ...
    for method_ind, method in enumerate(methods):
        ...
        xs = arg_x_func(extractor)
        ar_mean = extractor.get_ar_aggregated(np.mean)
        ar_min = extractor.get_ar_aggregated(min)
        lines.append(Line(xs, ar_mean, colors[method_ind], style='-', label=labels[method_ind]))
        lines.append(Line(xs, ar_min, colors[method_ind], style='--'))
