...
from src.plots.plot_general import colors, Line, plot_general, save_figure

assert addcopyfighandler, "Adds an option to copy figures by pressing Ctrl+C"


def plot_methods_ar_9_nodes_general(methods: list[str], labels: list[str], x_mean_func: callable, x_min_func: callable, **kwargs):
    """ Plots AR of different methods vs arg generated by the arg_x_func for 9 nodes. """
    lines = []
    for method_ind, method in enumerate(methods):
        extractor = DataExtractor(f'../../graphs/main/nodes_9/depth_3/output/{method}/out.csv')
        xs_mean = x_mean_func(extractor)
        ar_mean = extractor.get_ar_aggregated(np.mean)
        xs_min = x_min_func(extractor)
        ar_min = extractor.get_ar_aggregated(min)
        lines.append(Line(xs_mean, ar_mean, colors[method_ind], style='-', label=labels[method_ind]))
        lines.append(Line(xs_min, ar_min, colors[method_ind], style='--'))

    plot_general(lines, **kwargs)
...
def plot_ar_vs_p_heuristics_qaoa_attempts_1():
    ...
    plot_methods_ar_9_nodes_general(methods, labels, 0.52, 8)
    save_figure()
...
def plot_ar_vs_p_heuristics_ma_attempts_1():
    ...
    plot_methods_ar_9_nodes_general(methods, labels, 0.72, 5)
    save_figure()
