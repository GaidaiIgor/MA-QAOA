...
def plot_methods_9_nodes_general(methods: list[str], x_func: callable, y_func: callable, max_p: int = 10, transpose: bool = False, **kwargs):
    ...
    data = np.zeros((2, len(methods), max_p))
    for method_ind, method in enumerate(methods):
        extractor = DataExtractor(f'../../graphs/main/nodes_9/depth_3/output/{method}/out.csv')
        xs = list(x_func(extractor))[:max_p]
        ys = list(y_func(extractor))[:max_p]
        data[0, method_ind, :len(xs)] = xs
        data[1, method_ind, :len(ys)] = ys

    if transpose:
        data = np.transpose(data, [0, 2, 1])

    lines = data_matrix_to_lines(data, **kwargs)
...
def plot_ar_vs_p_heuristics_qaoa_attempts_1():
    ...
    plot_methods_9_nodes_general(methods, labels, 0.52, 8)
    save_figure()
...
def plot_ar_vs_p_heuristics_ma_attempts_1():
    ...
    plot_methods_9_nodes_general(methods, labels, 0.72, 5)
    save_figure()
...
def plot_fit_general(n_extrap: int, method_name: str, ds_param_name: str, ds_param: Sequence, method: str, aggregator_func: callable, fit_ind_range: Sequence, labels: Sequence):
    ...
    plot_general(rmse_lines, (ds_param_name, 'RMSE'), (1, None), legend_loc='best')
